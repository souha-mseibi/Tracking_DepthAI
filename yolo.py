...
def create_pipeline(stereo , yolo_config):
    ...
    if stereo:
        ...
        cam.preview.link(obj_det.input)
    ...
    # obj_det.setBlobPath(blobPath)
    # obj_det.setConfidenceThreshold(0.5)
    # obj_det.input.setBlocking(False)
    # obj_det.input.setQueueSize(1)
    # obj_det.setBoundingBoxScaleFactor(0.5)
    # obj_det.setDepthLowerThreshold(100)
    # obj_det.setDepthUpperThreshold(5000)
    # obj_det.setNumInferenceThreads(2)
    # obj_det.setNumNCEPerInferenceThread(1)
    # # Yolo specific parameters
    # obj_det.setNumClasses(5)
    # obj_det.setCoordinateSize(4)
    # obj_det.setAnchors([10.0,13.0,16.0,30.0,33.0,23.0,30.0,61.0,62.0,45.0,59.0,119.0,116.0,90.0,156.0,198.0,373.0,326.0])
    # obj_det.setAnchorMasks({ "side80": [0,1,2],"side40": [3,4,5],"side20": [6,7,8] })
    # obj_det.setIouThreshold(0.5)
    # obj_det.setConfidenceThreshold(0.5)
    ...
    obj_det.passthroughDepth.link(stereo_xout.input)
...
with dai.Device() as device:
    ...
    for name in ["color", "detection", "embedding" , "stereo"]:
    ...
    while True:
        ...
        if msgs is not None:
            ...
            for track in object_tracks:
                ...
                # cv2.rectangle(frame, (roi_plane[0], roi_plane[1]) , (roi_plane[2], roi_plane[3] ) , (0, 0, 255), cv2.FONT_HERSHEY_SIMPLEX)
                ...
                if stereo:
                    ...
                    coords = "Z: {:.2f} mm".format(detection.spatialCoordinates.z)
